...
- **Blockchain Integration**: Ethers.js v6.15.0 (primary), Web3.js v4.16.0 (frontend utilities)

### Frontend
- **Framework**: React 18 with TypeScript
- **Build Tool**: Vite 5.x (fast development & HMR)
- **Routing**: Wouter 3.x (lightweight React router)
...
- **ORM**: Drizzle ORM 0.39.x with Drizzle-Kit migrations
- **Schema Validation**: Drizzle-Zod for type-safe insert/select schemas
- **Session Store**: PostgreSQL-backed session storage
- **Document Storage**: Hybrid approach - critical hashes on blockchain, metadata in PostgreSQL

### Blockchain Layer
- **Smart Contracts**: Solidity ^0.8.20 (3 production contracts)
  - `BillOfLading.sol` - Digital eBL lifecycle management
  - `ShipmentTracking.sol` - Immutable event recording
  - `TradeFinance.sol` - Letter of Credit automation
- **Target Network**: BlockDAG (EVM-compatible DAG network)
- **RPC Endpoint**: `https://ide.awakening.bdagscan.com/`
- **Web3 Libraries**: ethers.js (backend), web3.js (frontend wallet connection)
- **Development Tools**: Hardhat-ready, Remix IDE compatible
- **Demo Mode**: Mock transaction generation for testing without blockchain

### DevOps & Tooling
- **Version Control**: Git + GitHub
- **Package Manager**: npm
- **TypeScript**: 5.6.3 (strict type checking)
- **Code Formatting**: Prettier (implicit via Vite)
- **Build Pipeline**: Vite build (frontend) + esbuild (backend bundling)
- **Environment Management**: `.env` pattern for secrets and configuration
- **Database Migrations**: `npm run db:push` (Drizzle Kit)
- **Development Server**: tsx (TypeScript execution), Vite dev server with HMR

### Key Dependencies
- **UI Framework**: Radix UI (accessible primitives)
...
## 🖼 Early UI Wireframe (Draft Concept)
...
├─────────────────────────────────────────────────┤
│  Consignee Address:   [0x742d...89Ab]          │
│  Carrier Address:     [0x9C3f...12Cd]          │
│  Vessel Name:         [MSC GÜLSÜN________]     │
│  Voyage Number:       [V425E___________]       │
│  Port of Loading:     [Shanghai, China___]     │
│  Port of Discharge:   [Rotterdam, NL_____]     │
│  Cargo Description:   [Electronics______]      │
│                                                  │
│  [Cancel]  [Save Draft]  [Issue to Blockchain] │
└─────────────────────────────────────────────────┘
```
...
```ascii
┌──────────────────────────────────────────────────┐
│  Shipment Tracking: SHIP-2024-001                │
├──────────────────────────────────────────────────┤
│  Timeline:                                        │
│  ✓ Container Loaded      (Shanghai, 2024-11-01) │
│  ✓ Vessel Departed       (Shanghai, 2024-11-02) │
│  ⏳ In Transit           (Pacific Ocean)         │
│  ○ Port Arrival          (Est. 2024-11-18)       │
│  ○ Customs Clearance                             │
│  ○ Delivered                                      │
│                                                   │
│  [+ Record New Event]  [Update Status]           │
└──────────────────────────────────────────────────┘
```
...
│                                                   │
│  Payment Milestones:                             │
│  ✓ Shipment Loaded        30% → $37,500 [Paid]  │
│  ⏳ Vessel Departed        40% → $50,000 [Pending]│
│  ○ Cargo Delivered        30% → $37,500          │
│                                                   │
│  [Accept Documents]  [Release Payment]           │
└──────────────────────────────────────────────────┘
```
...
│              │         │              │         │              │
│  React 18    │◄───────►│  Express API │◄───────►│  BlockDAG    │
│  TypeScript  │  HTTPS  │  Node.js     │  RPC    │  Network     │
│  Vite        │         │  TypeScript  │         │              │
└──────┬───────┘         └──────┬───────┘         └──────┬───────┘
       │                        │                        │
       │                        │                        │
...
│  │              │  │              │  │              │       │
│  │ • Landing    │  │ • BillOfLading│  │ • useAuth    │       │
│  │ • Dashboard  │  │   Card       │  │ • useQuery   │       │
│  │ • Documents  │  │ • Tracking   │  │ • useMutation│       │
│  │ • Finance    │  │   Timeline   │  │              │       │
│  │ • Explorer   │  │ • Sidebar    │  └──────────────┘       │
│  └──────────────┘  └──────────────┘                          │
│                                                                │
...
│                                                                │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐       │
│  │ Auth Layer   │  │  API Routes  │  │  Blockchain  │       │
│  │              │  │              │  │   Service    │       │
│  │ • Passport   │  │ • /api/auth  │  │              │       │
│  │ • OpenID     │  │ • /api/bills │  │ • ethers.js  │       │
│  │ • Sessions   │  │ • /api/track │  │ • Contract   │       │
│  │ • RBAC       │  │ • /api/finance│  │   ABIs       │       │
│  └──────────────┘  └──────────────┘  │ • RPC Client │       │
│                                       └──────────────┘       │
│                                                                │
│  ┌──────────────────────────────────────────────────┐        │
...
│                                                                │
│  ┌─────────────────────┐        ┌─────────────────────┐      │
│  │   PostgreSQL DB     │        │   BlockDAG Chain    │      │
│  │                     │        │                     │      │
│  │ • users             │        │ • BillOfLading.sol  │      │
│  │ • bills_of_lading   │        │ • Shipment         │      │
│  │ • shipments         │        │   Tracking.sol     │      │
│  │ • trade_finance     │        │ • TradeFinance.sol │      │
│  │ • transactions      │        │                     │      │
│  │ • sessions          │        │ Immutable Records   │      │
│  │                     │        │ Event Logs          │      │
│  │ Metadata & Cache    │        │ Smart Contract State│      │
│  └─────────────────────┘        └─────────────────────┘      │
└────────────────────────────────────────────────────────────────┘
```
...
1️⃣ Create Bill of Lading:
   User → Frontend Form → API POST /api/bills/create
        ...
        → Response → Frontend Update → User Notification

2️⃣ Track Shipment:
   User → Query Shipment → API GET /api/shipments/:id
        → Database Query + Blockchain Query (events)
        → Merge Data → Format Timeline
        → Response → Frontend Display

3️⃣ Issue Letter of Credit:
   Bank → Frontend Form → API POST /api/finance/issue-lc
        → Validation → Smart Contract (TradeFinance.sol)
        → Escrow Funds → Event Emission
        → Database Record → Notification to Beneficiary

4️⃣ Verify Document:
   User → Input Document Hash → API POST /api/verify
        → Blockchain Query (getDocument)
        → Compare Hashes → Return Verification Status
        → Frontend Display (✓ Verified / ✗ Invalid)
```
...
User → Click "Login with Replit"
     → Redirect to Replit OAuth
     → User Authorizes
     → Callback /api/callback
     → Passport.js Strategy Verify
     → Extract User Claims (email, sub, profile)
...
Protected Routes:
  Request → Check Session → Verify JWT Token
         → Refresh if Expired → Load User from DB
         → Check Role Permissions → Allow/Deny
```
...
└────────────────────────────────────────────────────────────┘
      ...
      │   (Static)   │ │  (Express)   │ │    (Neon)    │
      │              │ │              │ │              │
      │  Port 5000   │ │  Port 5000   │ │   Remote     │
      │  (Webview)   │ │  (Internal)  │ │   Serverless │
      └──────────────┘ └──────┬───────┘ └──────────────┘
                              │
                              │ HTTPS/WSS
                              ▼
